...
pub fn calculate_next_occurrence(
    ...
    let next = match recurrence.to_lowercase().as_str() {
        ...
        "monthly" => {
            ...
            let year = if current_due.month() == 12 {
                current_due.year() + 1
            } else {
                current_due.year()
            };
            let month = if current_due.month() == 12 {
                1
            } else {
                current_due.month() + 1
            };
        ...
        r if r.starts_with("every ") => parse_complex_recurrence(current_due, r)?,
        _ => {
            return Err(AppError::AiError(format!(
                "Unsupported recurrence format: {}",
                recurrence
            )))
        }
    };
...
fn parse_complex_recurrence(current_due: NaiveDate, r: &str) -> Result<NaiveDate, AppError> {
    ...
    let n: i64 = parts[1]
        .parse()
        .map_err(|_| AppError::AiError(format!("Invalid number in recurrence: {}", parts[1])))?;
    ...
    match unit.as_str() {
        ...
        "month" | "months" => {
            ...
            Ok(
                NaiveDate::from_ymd_opt(year, month as u32, current_due.day()).unwrap_or_else(
                    || {
                        let next_m = if month == 12 { 1 } else { month + 1 };
                        let next_y = if month == 12 { year + 1 } else { year };
                        NaiveDate::from_ymd_opt(next_y, next_m as u32, 1).unwrap()
                            - Duration::days(1)
                    },
                ),
            )
        }
        _ => Err(AppError::AiError(format!(
            "Unsupported unit in recurrence: {}",
            unit
        ))),
    }
}
...
pub fn poll_recurrences(app: &AppHandle, conn: &Connection) -> Result<(), AppError> {
    ...
    for (id, recurrence, next_occurrence_str) in generators {
        if let Ok(next_occurrence_date) =
            NaiveDate::parse_from_str(&next_occurrence_str, "%Y-%m-%d")
        {
            ...
            if let Ok(next_next_date) = calculate_next_occurrence(next_occurrence_date, &recurrence)
            {
                if let Ok(Some(task)) = crate::db::tasks::get_task(conn, &id) {
                    ...
                    crate::events::emit(
                        app,
                        crate::events::AppEvent::TaskUpdated { id: id.clone() },
                    );
